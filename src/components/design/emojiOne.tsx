...
const EmojiOne = ({ className, layerRef, topPos, leftPos }: Props) => {
    const one = getRandomColorFromGradient()
    const two = getRandomColorFromGradient()
    const three = getRandomColorFromGradient()
  return (
    <div
      className={`${className} ` }
      ref={layerRef}
      ...
      >
        ...
        <defs>
          ...
          >
            <stop
              stop-opacity=' 1'
              stop-color={`rgb(${one.r}, ${one.g}, ${one.b})`}
              offset='0'
            ></stop>
            <stop
              stop-opacity=' 1'
              stop-color={`rgb(${two.r}, ${two.g}, ${two.b})`}
              offset='0.48'
            ></stop>
            <stop
              stop-opacity=' 1'
              stop-color={`rgb(${three.r}, ${three.g}, ${three.b})`}
              offset='1'
            ></stop>
          </linearGradient>
        </defs>
